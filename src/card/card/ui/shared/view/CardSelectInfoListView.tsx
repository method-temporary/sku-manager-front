...
import { CardBundleFormModel } from 'cardbundle/present/logic/CardBundleFormModel';
...
@observer
@reactAutobind
class CardSelectInfoListView extends React.Component<Props> {
  //
  render() {
    //
    const {
                  ...
                  {cards && cards.length > 0 ? (
                    cards.map((cardWiths, index) => (
                      <Table.Row textAlign="center" key={cardWiths.card.id}>
                        ...
                        <Table.Cell>{moment(cardWiths.cardContents.registeredTime).format('YY.MM.DD')}</Table.Cell>
                        <Table.Cell>
...
export default CardSelectInfoListView;
