...
class ExcelReadModal extends React.Component<Props> {
  ...
  render() {
    const { open, onChangeOpen, fileName, uploadFile, onReadExcel, defaultForm, targetText } = this.props;
    return (
      <>
        <Modal size="tiny" open={open}>
          ...
          <Modal.Content>
            ...
            <p>500명 이상의 대량 등록의 경우 mySUNI LMS를 통해 등록을 요청해 주시기 바랍니다. </p>
            <Button
              primary
              as="a"
              download
              href={defaultForm ? baseUrl + `resources/${defaultForm}` : baseUrl + 'resources/학습자_등록_양식.xlsx'}
            >
              양식 다운로드
            </Button>
            {/*<Button basic*/}
