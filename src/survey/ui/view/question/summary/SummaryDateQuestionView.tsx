...
import { DateSummaryItems } from '../../../../analysis/model/DateSummaryItems';
...
@observer
export default class SummaryDateQuestionView extends React.Component<Props> {
  //
  render() {
    const { question, answerSummary, onToggleQuestion, lang } = this.props;

    const summaryItems = answerSummary.summaryItems as DateSummaryItems;
    let dateMap: string[] = [];
    let countMap: number[] = [];
    if (summaryItems.sentencesMap) {
      dateMap = Object.keys(summaryItems.sentencesMap);
      countMap = Object.values(summaryItems.sentencesMap);
    }
